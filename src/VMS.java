...
public class VMS {
    //este caracterizata de multimea
    //de campanii existente, precum si de utilizatorii care pot primi diverse vouchere
    ...
    public void updateCampaign(Integer id, Campaign campaign){
        ...
        if(aux.stat == Campaign.CampaignStatusType.NEW){
            ...
            if (campaign.getAvailableVouchers() != aux.getAvailableVouchers()){
                ...
                aux = campaign;
                System.out.println("Nu aveti voie sa modificati Available vouchers");
            }
        }

        if(aux.stat == Campaign.CampaignStatusType.STARTED){
            //aici se pot modifica doar numarul total de vouchere si data de finalizare
